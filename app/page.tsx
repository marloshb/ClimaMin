...
export default function Home() {
  ...
  const publishClimateRegimeSignal = () => {
    setFeed((items) => [{
      id: Date.now(),
      time: now.toLocaleTimeString("pt-BR", { hour12: false }),
      agent: "ENSO PUBLISHER",
      text: "ClimateRegimeSignal ENSO-026 publicado no Event Bus com incerteza, evidências e validade sazonal.",
      type: "ok" as Tone,
    }, ...items].slice(0, 8));
    setToast("ENSO-026 publicado · Torre, Perigos, Planejamento, Riscos e Continuidade notificados");
  };
      ...
      <section className="workspace-shell">
        ...
        <div className="workspace-heading">
          <div className="title-row">
            ...
            <div className="heading-actions">
              ...
              <button className="primary-button" onClick={activeKey === "climate" ? subview === "Super El Niño · ENSO" ? publishClimateRegimeSignal : publishClimateSignal : activeKey === "hazards" ? () => setSubview("Execuções") : activeKey === "twin" ? () => setSubview("Inspeções") : activeKey === "chain" ? () => setSubview("Restrições") : activeKey === "planning" ? () => setSubview("Cenários") : activeKey === "risk" ? () => setSubview("Stress Tests") : activeKey === "emergency" ? () => setSubview("Incidentes") : activeKey === "environment" ? () => setSubview("Ocorrências") : activeKey === "communications" ? () => setSubview("Redação Assistida") : activeKey === "data" ? () => setSubview("Incidentes") : activeKey === "governance" ? () => setSubview("Aprovações") : () => setDispatchOpen(true)}>{activeKey === "climate" ? subview === "Super El Niño · ENSO" ? "＋ Publicar regime" : "＋ Publicar sinal" : activeKey === "hazards" ? "＋ Novo model run" : activeKey === "twin" ? "＋ Nova vistoria" : activeKey === "chain" ? "＋ Nova restrição" : activeKey === "planning" ? "＋ Novo cenário" : activeKey === "risk" ? "＋ Novo stress test" : activeKey === "emergency" ? "＋ Declarar incidente" : activeKey === "environment" ? "＋ Nova ocorrência" : activeKey === "communications" ? "＋ Novo alerta" : activeKey === "data" ? "＋ Novo incidente" : activeKey === "governance" ? "＋ Nova aprovação" : "＋ Novo despacho"}</button>
            </div>
          </div>
          {activeKey === "control" || activeKey === "climate" || activeKey === "hazards" || activeKey === "twin" || activeKey === "chain" || activeKey === "planning" || activeKey === "risk" || activeKey === "emergency" || activeKey === "environment" || activeKey === "communications" || activeKey === "data" || activeKey === "governance" ? <div className="operational-context-bar"><div className="horizon-control"><span>HORIZONTE</span>{(activeKey === "climate" ? subview === "Super El Niño · ENSO" ? ["SON", "OND", "NDJ", "DJF", "MAM", "12M", "ANÁLOGOS"] : ["AGORA", "+6H", "+24H", "+72H", "15D", "6M", "2050"] : activeKey === "planning" ? ["AGORA", "+6H", "+12H", "+24H", "+48H", "+72H", "7D", "CENÁRIO"] : activeKey === "risk" ? ["AGORA", "+24H", "1Y", "2030", "2050", "EVENTO", "STRESS"] : activeKey === "emergency" ? ["AGORA", "+30M", "+2H", "+6H", "+12H", "OP-01", "RECOVERY"] : activeKey === "environment" ? ["AGORA", "+1H", "+3H", "+6H", "+12H", "EVENTO", "RECOVERY"] : activeKey === "communications" ? ["AGORA", "+15M", "+1H", "+3H", "+6H", "EVENTO", "PÓS-EVENTO"] : activeKey === "data" ? ["AGORA", "+5M", "+30M", "+2H", "+6H", "INCIDENTE", "RECOVERY"] : activeKey === "governance" ? ["AGORA", "DECISÃO", "EVENTO", "30D", "90D", "AUDIT", "REPLAY"] : activeKey === "hazards" || activeKey === "twin" || activeKey === "chain" ? ["AGORA", "+1H", "+3H", "+6H", "+12H", "+24H", "CENÁRIO"] : ["AGORA", "+6H", "+24H", "+72H", "7D", "30D", "CENÁRIO"]).map((item) => <button className={horizon === item ? "active" : ""} key={item} onClick={() => { setHorizon(item); setToast(`Contexto sincronizado em ${item}: mapa, gráficos, probabilidades e briefing`); }}>{item}</button>)}</div><div className="context-summary"><span>{activeKey === "climate" ? subview === "Super El Niño · ENSO" ? "ClimateRegimeContext" : "ClimateContext" : activeKey === "hazards" ? "HazardContext" : activeKey === "twin" ? "AssetContext" : activeKey === "chain" ? "ChainContext" : activeKey === "planning" ? "ScenarioContext" : activeKey === "risk" ? "RiskContext" : activeKey === "emergency" ? "IncidentContext" : activeKey === "environment" ? "TerritorialContext" : activeKey === "communications" ? "CommunicationContext" : activeKey === "data" ? "DataQualityContext" : activeKey === "governance" ? "GovernanceContext" : "OperationalContext"}</span><strong>Tubarão · {horizon} · {scenarioName}</strong><small>{profile} · {activeKey === "climate" ? subview === "Super El Niño · ENSO" ? "ENSO-026 · RONI · VERY_STRONG_CANDIDATE" : "FR-2204" : activeKey === "hazards" ? "HMR-882 · HS-882" : activeKey === "twin" ? `${selectedAsset} · HS-882` : activeKey === "chain" ? `PR-881 · BOT-022 · ${selectedAsset}` : activeKey === "planning" ? "SCN-0184 · PLN-v18 → ALT-B" : activeKey === "risk" ? "RS-041 · R-021 · FI-021" : activeKey === "emergency" ? "AL-0187 · ACT-019 · INC-024" : activeKey === "environment" ? "TS-081 · C-07 · OCC-GROUP-018" : activeKey === "communications" ? "COM-2026-018 · ALT-118 · TS-081" : activeKey === "data" ? "GW-03 · NIV-04 · 147/153 sources" : activeKey === "governance" ? "DEC-0248 · DS-0248 · POL-OPS-17" : selectedAsset} · {stage.label}</small></div></div> : null}
        ...
        <div className="workspace-scroll">
          ...
          {activeKey === "climate" ? <ClimateModule
            ...
            onClimateRegimeSignal={publishClimateRegimeSignal}
            onHazards={() => { selectWorkspace("hazards"); setToast("Forecast Run FR-2204 encaminhado ao Módulo 3 · Perigos"); }}
            onOpenModule={(module, targetSubview) => { selectWorkspace(module); setSubview(targetSubview); setToast(`ENSO-026 conectado a ${targetSubview} · contexto, incerteza e linhagem preservados`); }}
          /> : null}
